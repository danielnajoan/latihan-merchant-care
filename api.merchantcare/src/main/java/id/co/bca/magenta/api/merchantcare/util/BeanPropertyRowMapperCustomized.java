...
/**
 * Untuk siapapun yang bikin class ini, maaf yak kerjaannya di customeized :D
 * Creditnya masih ada kok di bawah :)
 */
 ...
 */
public class BeanPropertyRowMapperCustomized<T> implements RowMapper<T> {
//	private static final Logger logger = Logger.getLogger(BeanPropertyRowMapperCustomized.class);
	...
	/**
	 * Create a new {@code BeanPropertyRowMapperCustomized} for bean-style configuration.
	 * @see #setMappedClass
	 * @see #setCheckFullyPopulated
	 */
	public BeanPropertyRowMapperCustomized() {
	}

	/**
	 * Create a new {@code BeanPropertyRowMapperCustomized}, accepting unpopulated
	 * properties in the target bean.
	 * <p>Consider using the {@link #newInstance} factory method instead,
	 * which allows for specifying the mapped type once only.
	 * @param mappedClass the class that each row should be mapped to
	 */
	public BeanPropertyRowMapperCustomized(Class<T> mappedClass) {
		initialize(mappedClass);
	}

	/**
	 * Create a new {@code BeanPropertyRowMapperCustomized}.
	 * @param mappedClass the class that each row should be mapped to
	 * @param checkFullyPopulated whether we're strictly validating that
	 * all bean properties have been mapped from corresponding database fields
	 */
	public BeanPropertyRowMapperCustomized(Class<T> mappedClass, boolean checkFullyPopulated) {
		initialize(mappedClass);
	...
	public T mapRow(ResultSet rs, int rowNumber) throws SQLException {
		...
		for (int index = 1; index <= columnCount; index++) {
			...
			String fieldTmp = field.replace("_", "");
//			PropertyDescriptor pd = (this.mappedFields != null ? this.mappedFields.get(field) : null);
			PropertyDescriptor pd = (this.mappedFields != null ? this.mappedFields.get(fieldTmp) : null);
			if (pd != null) {
				try {
	...
	/**
	 * Static factory method to create a new {@code BeanPropertyRowMapperCustomized}
	 * (with the mapped class specified only once).
	 * @param mappedClass the class that each row should be mapped to
	 */
	public static <T> BeanPropertyRowMapperCustomized<T> newInstance(Class<T> mappedClass) {
		return new BeanPropertyRowMapperCustomized<>(mappedClass);
	}
